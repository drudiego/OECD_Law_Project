module.exports.names = [
    "Dutch NCP_Oxfam Novi, Green Peace Netherlands et al against ING",
    "Dutch NCP_BWI+IKEA_Final Statement",
    "Dutch NCP_Odoh family Nigeria vs. SPDC_initial",
    "Dutch NCP_FNV vs. Just Eat",
    "Dutch NCP_UNI vs. Ikea",
    "Dutch NCP_BWI et al vs. Inter Ikea Holding_initial",
    "Dutch NCP_Shehri-CBE vs, Makro-Habib Pakistan",
    "Dutch NCP_FoE vs. Shell Niger Delta",
    "Dutch NCP_Aminigboko Community vs. Shell Petroleum Development",
    "Dutch NCP_FNV Bondgenoten vs. Plaid Netherland",
    "Dutch NCP_SOMO et al vs. Nidera Holding",
    "Dutch NCP_Fenceline Community vs. Pilipinas Shell Petroleum ",
    "Dutch NCP_Amnesty International and FoE against Royal Dutch She",
    "Dutch NCP_FoE against ING",
    "Dutch NCP_Bralima employees against Heineken",
    "Dutch NCP_Former employees against Philips Lighting",
    "Dutch NCP_SOMO Both Ends against ABP/APG",
    "Dutch NCP_SEW against Royal Dutch Shell",
    "Dutch NCP_FNV et al against Chevron",
    "Dutch NCP_IUF against PvM",
    "Dutch NCP_UNI against VEON",
    "Dutch NCP_Obelle against Shell Petroleum Development",
    "Dutch NCP_Bart Stapert against Mylan",
    "Dutch NCP_FoE against Rabobank",
    "Dutch NCP_ENDs et al against Atradius Dutch State Business",
    "Dutch NCP_ENDs et al against Atradius Dutch State Business",
    "Dutch NCP_FIVAS et al against Bresser"

]



module.exports.linkedHumanRightsIssue = [
    "Child labour",
    "Labour rights violation",
    "Forced labour",
    "Land grabbing",
    "Gender harassment"
]

module.exports.corporateInvolvementToTheHarmsAndAbuses = [
    "Caused",
    "Contributed",
    "Directly linked"
]

//Not sure how this is going to be implemented
// module.exports.definingRisksHarmsAndAbuse

module.exports.corporateResponsibilityInUNGPsFramework = [
    "HRDD",
    "Human rights policy",
    "Stakeholder engagement",
    "Mitigations",
    "Remediation",
    "Transparency/communication"
]

module.exports.specificCorporateHumanRightsResponsibilities = [
    "Conflict-affected zones",
    "Supply chain",
    "Climate change/environment"
]

module.exports.actors = [
    "Consulting company",
    "Audits",
    "SMEs",
    "Tech companies",
    "Parent companies",
    "Investors/Financial Institutions",
    "State responsibility"
]

module.exports.otherSubstantialMatters = [
    "Conflict with domestic laws",
    "Corporate governance",
    "Gaps in OECD guidelines",
    "SLAPP"
]

module.exports.NCPsEffortsToResolveTheCases = [
    "Hired independent mediator",
    "Hired external consultants",
    "Sought advice from other parties",
    "Hired translators",
    "Involved victims in the process",
    "Monitored and followed the companies actions"
]

module.exports.NCPGoodOffices = [
    "Mediation",
    "Conciliation"
]

module.exports.challengesDurigTheGoodOffice = [
    "Delay",
    "Breach of confidentiality",
    "Language barriers",
    "No cooperation between NCPs",
    "Parallel proceedings"
]

module.exports.outcomesResultOfTheMediation = [
    "Changes/updates of the company policy",
    "Written agreement between parties",
    "Financial compensations",
    "Disengagement/Case of operation",
    "Acknowledgements"
]


module.exports.analysisOnStatement = [
    "Determination of compliance/non-compliance",
    "Referred instruments",
    "Referred other NCP decisions",
    "Analysed OECD Guidelines"
]

module.exports.location = [
    "Netherlands",
    "Germany",
    "Brazil",
    "France",
    "Peru",
    "Nigeria",
    "Taiwan",
    "USA",
    "Mexico",
    "Belgium",
    "New Zealand",
    "Mongolia",
    "Saudi Arabia"
]