...
module.exports.renderHandbook = (req, res) => {
	res.render("statements/handbook");
};
...
module.exports.search = async (req, res, next) => {
	...
	for (const filter in selectedFilters) {
		...
		if (selectedFilters[filter].length > 0) {
			selectedFilters[filter] = Array.isArray(selectedFilters[filter])
		...
		matchingSegmentIds[filter] = matchingSegments[filter].map(
			(segment) => segment._id
		);
	}
	...
	if (Object.keys(matchingSegments).length > 0) {
		const finalQuery = Object.values(matchingSegmentIds).map((ids) => ({
			segments: { $in: ids },
		}));
...
module.exports.updateStatement = async (req, res) => {
	...
	if (req.body.deleteFiles) {
		...
		await statement.updateOne({
			$pull: { pdfFile: { filename: { $in: req.body.deleteFiles } } },
		});
	}
